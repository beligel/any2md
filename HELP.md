...
5. В окне **Лог** отображается ход работы, а в статусной строке — прогресс `N из M` файлов.
...
| `--list-formats` | Список форматов | `--list-formats` |
| `-o -` | Вывод в stdout | `-o -` |
...
- Для OCR: bundled Tesseract с языками eng/rus/fra/deu/spa скачивается автоматически (опционально можно использовать системный `tesseract-ocr`)
