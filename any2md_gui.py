...
Основные опции CLI
  -o, --output        Путь для сохранения ('-' для stdout)
...
class Any2MdApp:
    ...
    def _update_status(self, current: int, total: int):
        self.status.configure(text=f"Обработано {current} из {total} файлов")
        ...
        def worker():
            try:
                result = convert(
                    ...
                    language=language,
                    progress_callback=lambda c, t: self.root.after(0, lambda: self._update_status(c, t)),
                )
